...
class HlsjsP2PWrapperPrivate {
    ...
    constructor(p2pConfig, hlsjs, PeerAgentModuleConstructor) {
        ...
        hlsjs.on(hlsjs.constructor.Events.ERROR, this._onMediaEngineError.bind(this));
    }
    ...
    _onMediaEngineError(event, data) {
        ...
        if (data.fatal) {
            if (this.peerAgentModule && this.peerAgentModule.onPlaybackError) {
                this.peerAgentModule.onPlaybackError(`${data.type} - ${data.details}`);
            }
    ...
    _overrideABRController(hlsjs) {
        ...
        if (hlsjs.abrController._findBestLevel) {
            ...
            console.error('Could not find ABRController method _findBestLevel or findBestLevel');
            return;
        }
        ...
        const newFindLevel = function() {
            ...
            if (self.peerAgentModule && self.peerAgentModule.nextSegmentPrebuffered && self.peerAgentModule.nextSegmentPrebuffered.video && level < currentLevel) {
                level = currentLevel;
            }
            return level;
        };
