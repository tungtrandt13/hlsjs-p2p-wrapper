...
class HlsjsP2PWrapperPrivate {
    ...
    newMediaEngine(hlsjsConfig = {}) {
        ...
        let newDefaultConf = this.getConfig();
        if (hlsjsConfig.liveSyncDurationCount !== undefined) {
            delete newDefaultConf.liveSyncDuration; // Don't override liveSyncDuration if liveSyncDurationCount has been manually set by user
        }
        return new Hlsjs(defaults(hlsjsConfig, newDefaultConf));
    }
