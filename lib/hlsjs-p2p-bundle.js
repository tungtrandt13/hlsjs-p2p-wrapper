...
class StreamrootHlsjsBundle {
    constructor (hlsjsConfig, p2pConfig) {
        let defaultHlsjsConfig = this._defaultHlsjsConfig;
        if (hlsjsConfig.liveSyncDurationCount !== undefined) {
            delete defaultHlsjsConfig.liveSyncDuration; // Don't override liveSyncDuration if liveSyncDurationCount has been manually set by user
        }

        let hlsjs = new Hlsjs(defaults(hlsjsConfig, defaultHlsjsConfig));
        new HlsjsP2PWrapper(p2pConfig, hlsjs);

        return hlsjs;
    }

    get _defaultHlsjsConfig() {
        return {
            maxBufferSize: 0,
            maxBufferLength: 30,
            liveSyncDuration: 30,
