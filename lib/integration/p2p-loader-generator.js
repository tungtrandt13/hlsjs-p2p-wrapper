...
function extendHeadersFromXhrSetup(xhrSetupFunction, headers) {
    headers = headers || {};
    var noEventHandlers = () => { throw new Error('Can\'t use event handler property in this xhrSetup function. Contact Streamroot support.'); }
    ...
    return class P2PLoader {
        ...
        constructor(config) {
            if (config) {
                ...
                this.headers = extendHeadersFromXhrSetup(this.xhrSetup, config.request && config.request.headers ? config.request.headers : {});
            }
            this.reset();
        }
