...
function extendHeadersFromXhrSetup(xhrSetupFunction, headersBase) {
    ...
    xhrSetupFunction && xhrSetupFunction(xhrMock);
    ...
    return class P2PLoader {
        ...
        constructor(config) {
            if (config) {
                ...
                this.headers = extendHeadersFromXhrSetup(config.xhrSetup, config.request && config.request.headers ? config.request.headers : {});
            }
            this.reset();
        }
