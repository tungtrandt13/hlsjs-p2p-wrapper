...
class SegmentView {
    ...
    static fromArrayBuffer(arrayBuffer) {
        var u32Data = new Uint32Array(arrayBuffer),
            [level, urlId, sn, bitrate] = u32Data;

        return new SegmentView({
            trackView: new TrackView({ level, urlId, bitrate }),
            sn
        });
    }
     ...
     */
    toArrayBuffer() {
        const { level, urlId, bitrate } = this.trackView;

        return new Uint32Array([level, urlId, this.sn, bitrate]).buffer;
    }
